...
from utils.bot_utils import parsing, txt_reader, keyboard_markup, config

from utils.artificial_intelligence.ai import get_city_prediction
...
async def command_start(message: types.Message):
    await bot.send_message(message.from_user.id, 'Привет, {0.first_name}!'.format(message.from_user),
                           reply_markup=keyboard_markup.mainMenu)


@dp.message_handler()
async def bot_message(message: types.Message):
    if message.text == "Информация о COVID-19":
        await bot.send_message(message.from_user.id, "Информация о COVID-19", reply_markup=keyboard_markup.otherMenu2)
    elif message.text == "Сколько людей заражено за сегодня":
        await bot.send_message(message.from_user.id, f'Сегодня заразились {parsing.parser()} человек.')
        ...
        await bot.send_message(message.from_user.id, "Интересное о COVID-19", reply_markup=keyboard_markup.otherMenu)
    elif message.text == "Факты о COVID-19":
        await bot.send_message(message.from_user.id, txt_reader.arbitrary())
    elif message.text == "Назад":
        await bot.send_message(message.from_user.id, "Интересное о COVID-19", reply_markup=keyboard_markup.mainMenu)
    elif message.text == "Профилактика COVID-19":
        await bot.send_message(message.from_user.id, "Профилактика COVID-19", reply_markup=keyboard_markup.preventionMenu)
